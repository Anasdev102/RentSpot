...
function DonutChart({ revenue, data }) {
  return (
...
function OperationsSnapshot({ stats }) {
  return (
...
function PopularStadiums({ reservations }) {
...
export default function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    return [
      ['Total Users', data.stats.total_users, '+12%', Users, 'from-blue-50 to-blue-100 text-primary'],
      ['Total Stadiums', data.stats.total_stadiums, '+8%', Trophy, 'from-blue-50 to-cyan-100 text-primary'],
      ['Total Reservations', data.stats.reservations, '+15%', CalendarDays, 'from-green-50 to-emerald-100 text-secondary'],
      ['Total Revenue', `$${Number(data.stats.revenue || 0).toLocaleString()}`, '+20%', CreditCard, 'from-amber-50 to-yellow-100 text-gold'],
    ];
  }, [data]);
  ...
  const operationStats = useMemo(() => {
    ...
    return {
      pendingReservations: data.recent_reservations.filter((reservation) => reservation.status === 'pending').length,
      confirmedReservations: data.recent_reservations.filter((reservation) => reservation.status === 'confirmed').length,
      paidPayments: data.recent_reservations.filter((reservation) => reservation.payment?.status === 'paid').length,
    };
  }, [data]);
  ...
  const reservationChartData = useMemo(() => {
    ...
    const total = Number(data.stats.reservations || 0);
  ...
  const revenueChartData = useMemo(() => {
    ...
    const paid = data.recent_reservations.filter((reservation) => reservation.payment?.status === 'paid').length;
    const unpaid = data.recent_reservations.filter((reservation) => reservation.payment?.status !== 'paid').length;
    const completed = data.recent_reservations.filter((reservation) => reservation.status === 'completed').length;
  ...
  return (
    <div className="space-y-6 text-slate-900">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <RecentReservations reservations={data.recent_reservations} />
        <PopularStadiums reservations={data.recent_reservations} />
      </section>
    </div>
  );
}
