...
export default function AdminNotifications() {
  ...
  const openNotification = async (notification) => {
    ...
    setOpen(false);
    navigate(notification.target_url || '/admin');
  };
  ...
  return (
    <div ref={panelRef} className="relative">
      <button
        ...
        className={`notification-trigger relative grid h-12 w-12 place-items-center rounded-2xl border border-slate-200 bg-white text-slate-700 shadow-sm transition-all duration-300 ease-out hover:border-primary hover:text-primary ${open ? 'is-open border-primary text-primary ring-4 ring-primary/10' : ''}`}
        aria-label="Notifications"
      >
        <Bell size={21} />
        {data.unread_count > 0 && (
          <span className="notification-badge absolute -right-1 -top-1 grid min-h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1.5 text-[11px] font-black text-white ring-2 ring-white">
            {data.unread_count}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="panel-pop absolute right-0 z-50 mt-3 w-[360px] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_60px_rgba(15,35,65,0.16)] transition-all duration-300 ease-out">
