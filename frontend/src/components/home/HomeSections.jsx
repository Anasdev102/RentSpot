...
const featuredSports = [
  ...
  ['Tennis', 'Tennis courts', Trophy, 'text-primary', 'bg-primary/10'],
...
const heroBadges = [
  ...
  [BadgeCheck, 'Instant Confirmation'],
];

const howItWorks = [
  [Sparkles, 'Choose a Field', 'Select your favorite sport and choose a field.'],
  [CalendarCheck, 'Select Date & Time', 'Pick the best date and time that suits you.'],
  [CreditCard, 'Pay Online & Play', 'Pay securely online and enjoy your game.'],
];

const reasons = [
  [ShieldCheck, 'Real-time Availability', 'Check real-time availability and book instantly.', 'text-primary'],
  [Lock, 'Secure Payment', '100% secure online payment with suitable options.', 'text-secondary'],
  [Sparkles, 'Easy Booking', 'Quick and easy booking in just a few steps.', 'text-primary'],
  [Medal, 'Trusted Fields', 'High quality fields and top rated by players.', 'text-gold'],
];
    ...
    <section className="bg-white py-10">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          {stadiums.length === 0 && <p className="text-muted">No stadiums available yet.</p>}
        </div>
      </div>
    </section>
  );
      ...
      <div className="grid gap-8 rounded-lg bg-primary/5 p-8 md:grid-cols-[0.8fr_1.2fr] md:p-10">
        ...
        <form onSubmit={onSubmit} className="grid gap-4 rounded-lg bg-white p-6 shadow-soft">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <input className="input" placeholder="Full Name" value={contact.name} onChange={(event) => onContactChange({ ...contact, name: event.target.value })} required />
            <input className="input" placeholder="Email Address" type="email" value={contact.email} onChange={(event) => onContactChange({ ...contact, email: event.target.value })} required />
          </div>
          ...
          <input className="input" placeholder="Subject" value={contact.subject} onChange={(event) => onContactChange({ ...contact, subject: event.target.value })} />
          <textarea className="input min-h-32" placeholder="Your Message" value={contact.message} onChange={(event) => onContactChange({ ...contact, message: event.target.value })} required />
